...
import ExpertCategory from '../../components/expert-signup/ExpertCategory';
...
function ExpertSignUp() {
  ...
  // 누적 요청서, 등록된 전문가, 평균 리뷰별점을 어떻게 select하여 가져오는 지에 따라 다른 형태 - 3 항목의 갯수만 가져오면됨.

  const [expert, setExpert] = useState([
    { expertProfile: '', expertCategories: '', expertReview: '' },
  ]);
  // 전문가 테이블에서 프로필 사진, 카테고리 가져오기. 전문가 리뷰는 전문가들이 직접 작성
  useEffect(() => {
    fetch(
      SERVER_PORT +
        '누적 요청서의 총 갯수, 등록된 전문가의 모든 수, 평균 리뷰별점 API',
      {
        method: 'GET',
      }
    )
      ...
      <div className={styles.statisticContainer}>
        <div className={styles.statisticCaption}>
          ...
          <p className={styles.caption}>
            많은 고객들이 조건에 딱! 맞는 전문가를 찾고 있어요. 고객에게 필요한
            딱! 맞는 전문가가 되어보세요.
          </p>
        </div>
      ...
      <div className={styles.Info}>
        ...
        <ul className={styles.infoStage}>
          <li className={styles.infoCaption}>
            ...
            고객이 필요한 서비스의 요청서를 작성하면 숨고가 검토 후 전문가님께
            무료로 보내드려요.
          </li>
          <li className={styles.infoCaption}>
            <img
