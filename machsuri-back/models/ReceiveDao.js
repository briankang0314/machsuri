...
const getReceive = async (userId) => {
  ...
  lc.name as category, UNIX_TIMESTAMP(request_form.ended_at) as ended_at, request_form.minor_category_id,
  ANY_VALUE(DATE_FORMAT(request_form.created_at,'%Y-%m-%d')) as created_at, lc.image
  from request_form
  left join minor_categories lc on lc.id = request_form.minor_category_id
  where request_form.user_id=${userId}
  group by request_form.minor_category_id, request_form.ended_at
  `;
};
