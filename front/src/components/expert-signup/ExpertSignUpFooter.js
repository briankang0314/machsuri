...
import styles from './ExpertSignUpFooter.module.scss';
import { SERVER_PORT } from '../../config';

function ExpertSignUpFooter({
  setFormPage,
  pageNumber,
  allData,
  checkMinor,
  ...
  const sendExpertInfo = (data, router) => {
    fetch(`${SERVER_PORT}/expert/${router}`, {
      method: 'POST',
          ...
          onClick={e => {
            ...
            if (e.target.innerText === '가입하기') {
              if (
                ...
                // fetch 보내기
                sendExpertInfo(allData.current, 'signupdirect');
              } else if (
...
export default ExpertSignUpFooter;
